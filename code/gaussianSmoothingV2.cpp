...
/*!
	...
	Why or why not?
	Answer : there is not much difference in intensities but space occupancy is little bit more
	in 11-by-11 because it  might be clear from formula. Visually can be also be seen by applying
	threshold
*/
...
int main( int argc, char **argv ) {
	...
	int i,j;
	for( i = 0; i < 10; i++ )
        for ( j = 0; j < 10; j++ ) pixel[ ( 50 + i ) * image->widthStep + ( 50 + j ) ] = 255;
...
//    pixel[ 51 * image->widthStep + 54 ] = 255;
	...
	// cvThreshold( output, output, 0, 255, CV_THRESH_BINARY );
