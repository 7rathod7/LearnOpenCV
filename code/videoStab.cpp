...
int main( int argc, char **argv ) {
	...
	//Video Load   
	CvCapture * capture = cvCreateFileCapture( argv[1] ); //cvCaptureFromCAM(0); //cvCreateFileCapture("1.avi");   

	//Window   
	cvNamedWindow( "Origin OpticalFlow" , CV_WINDOW_AUTOSIZE );
	...
	while(1) {      
		...
		if(one_zero == 0 ){   
			...
			//display
			cvNamedWindow("Stabilizing", CV_WINDOW_AUTOSIZE );
